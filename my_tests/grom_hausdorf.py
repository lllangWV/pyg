...
def adjacency_matrix_knn(points, k):
    # Calculate the k-NN graph
    knn_graph = kneighbors_graph(points, k, mode='connectivity', include_self=False)
    # Make the graph symmetric (i.e., if i is a neighbor of j, then j is a neighbor of i)
...
if __name__=='__main__':
    ...
    ps_3=my_lib.math.rot_z(ps_1,45)

    adj_1=adjacency_matrix_knn(ps_1,k=3)
    adj_2=adjacency_matrix_knn(ps_2,k=3)
    adj_3=adjacency_matrix_knn(ps_3,k=3)

    print(adj_1)
    print(adj_2)
    gh=gromov_hausdorff(AG=adj_1, AH=adj_2, mapping_sample_size_order=DEFAULT_MAPPING_SAMPLE_SIZE_ORDER)
    print(gh)
    ...
    for i,shape_name_i in enumerate(shape_names_custom):
        for j,shape_name_j in enumerate(shape_names_custom):
            ...
            adj_1=adjacency_matrix_knn(point_set_1,k=3)
            adj_2=adjacency_matrix_knn(point_set_2,k=3)

            print(gromov_hausdorff(AG=adj_1, AH=adj_2, mapping_sample_size_order=DEFAULT_MAPPING_SAMPLE_SIZE_ORDER)[0])
            # similarity_matrix[i,j]=gromov_hausdorff(AG=adj_1, AH=adj_2, mapping_sample_size_order=DEFAULT_MAPPING_SAMPLE_SIZE_ORDER)[0]
